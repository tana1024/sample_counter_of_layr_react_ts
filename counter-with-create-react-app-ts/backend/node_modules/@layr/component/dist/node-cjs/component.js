 ...
 * ```
 */
class Component extends observable_1.Observable(Object) {
    // === Creation ===
    /**
    ...
    constructor(object = {}) {
        ...
        for (const attribute of this.getAttributes()) {
            ...
            let value;
            if (core_helpers_1.hasOwnProperty(object, name)) {
                value = object[name];
            }
            else {
    ...
    static create(object = {}, options = {}) {
        ...
        const fullAttributeSelector = properties_1.mergeAttributeSelectors(attributeSelector, properties_1.createAttributeSelectorFromNames(Object.keys(object)));
        return possibly_async_1.possiblyAsync(possibly_async_1.possiblyAsync.forEach(component.getAttributes({ attributeSelector: fullAttributeSelector }), (attribute) => {
            return possibly_async_1.possiblyAsync(attributeFilter !== undefined ? attributeFilter.call(component, attribute) : true, (isNotFilteredOut) => {
                if (isNotFilteredOut) {
                    const name = attribute.getName();
                    let value;
                    if (core_helpers_1.hasOwnProperty(object, name)) {
        ...
        }), () => (initialize ? possibly_async_1.possiblyAsync(component.initialize(), () => component) : component));
    }
    // === Initialization ===
    /**
    ...
    static setComponentName(name) {
        utilities_1.assertIsComponentName(name);
    ...
    static getBaseComponentType() {
        return utilities_1.getComponentClassTypeFromComponentName(this.getBaseComponentName());
    }
    getBaseComponentType() {
        return utilities_1.getComponentInstanceTypeFromComponentName(this.constructor.getBaseComponentName());
    }
    /**
    ...
    static getComponentType() {
        return utilities_1.getComponentClassTypeFromComponentName(this.getComponentName());
    }
    /**
    ...
    static getPropertyClass(type) {
        if (type === 'Property') {
            return properties_1.Property;
        }
        if (type === 'Attribute') {
            return properties_1.Attribute;
        }
        if (type === 'PrimaryIdentifierAttribute') {
            return properties_1.PrimaryIdentifierAttribute;
        }
        if (type === 'SecondaryIdentifierAttribute') {
            return properties_1.SecondaryIdentifierAttribute;
        }
        if (type === 'Method') {
            return properties_1.Method;
        }
    ...
    setProperty(name, PropertyClass, propertyOptions = {}) {
        ...
        if (property === undefined) {
            ...
            if (core_helpers_1.getTypeOf(property) !== core_helpers_1.getTypeOf(PropertyClass.prototype)) {
                throw new Error(`Cannot change the type of a property (${property.describe()})`);
            }
            property.setOptions(propertyOptions);
        }
        if (properties_1.isAttributeClass(PropertyClass)) {
            const descriptor = {
                configurable: true,
                enumerable: true,
                get() {
    ...
    deleteProperty(name) {
        const properties = this.__getProperties();
        if (!core_helpers_1.hasOwnProperty(properties, name)) {
            return false;
        }
    ...
    __getProperties({ autoCreateOrFork = true } = {}) {
        if (autoCreateOrFork) {
            ...
            else if (!core_helpers_1.hasOwnProperty(this, '__properties')) {
    ...
    __getAttribute(name, options) {
        ...
        if (!properties_1.isAttributeInstance(property)) {
    ...
    setAttribute(name, attributeOptions = {}) {
        return this.setProperty(name, properties_1.Attribute, attributeOptions);
    }
    /**
    ...
    __traverseAttributes(iteratee, { attributeSelector, setAttributesOnly }) {
        for (const attribute of this.getAttributes({ attributeSelector })) {
            ...
            const name = attribute.getName();
            const subattributeSelector = properties_1.getFromAttributeSelector(attributeSelector, name);
    ...
    __getIdentifierAttribute(name, options) {
        ...
        if (!properties_1.isIdentifierAttributeInstance(property)) {
    ...
    __getPrimaryIdentifierAttribute(options) {
        ...
        for (const identifierAttribute of this.getIdentifierAttributes({ autoFork })) {
            if (properties_1.isPrimaryIdentifierAttributeInstance(identifierAttribute)) {
                return identifierAttribute;
            }
        }
        return undefined;
    ...
    setPrimaryIdentifierAttribute(name, attributeOptions = {}) {
        return this.setProperty(name, properties_1.PrimaryIdentifierAttribute, attributeOptions);
    }
    /**
    ...
    __getSecondaryIdentifierAttribute(name, options) {
        ...
        if (!properties_1.isSecondaryIdentifierAttributeInstance(property)) {
    ...
    setSecondaryIdentifierAttribute(name, attributeOptions = {}) {
        return this.setProperty(name, properties_1.SecondaryIdentifierAttribute, attributeOptions);
    }
    /**
    ...
    __partitionAttributes(object) {
        ...
        const identifierAttributeSelector = properties_1.createAttributeSelectorFromAttributes(this.getIdentifierAttributes());
        for (const [name, value] of Object.entries(object)) {
            if (properties_1.getFromAttributeSelector(identifierAttributeSelector, name) === true) {
    ...
    static normalizeIdentifierDescriptor(identifierDescriptor) {
        ...
        if (!core_helpers_1.isPlainObject(identifierDescriptor)) {
            throw new Error(`An identifier descriptor should be a string, a number, or an object, but received a value of type '${core_helpers_1.getTypeOf(identifierDescriptor)}' (${this.describeComponent()})`);
        }
    ...
    static getIdentityMap() {
        if (this.__identityMap === undefined) {
            Object.defineProperty(this, '__identityMap', { value: new identity_map_1.IdentityMap(this) });
        }
        else if (!core_helpers_1.hasOwnProperty(this, '__identityMap')) {
    ...
    resolveAttributeSelector(attributeSelector, options = {}) {
        attributeSelector = properties_1.normalizeAttributeSelector(attributeSelector);
    ...
    __resolveAttributeSelector(attributeSelector, options) {
        ...
        const isEmbedded = utilities_1.isComponentInstance(this) && this.constructor.isEmbedded();
        ...
        if (!_isDeep || includeReferencedComponents || isEmbedded) {
            for (const attribute of this.getAttributes({ filter, setAttributesOnly })) {
                ...
                if (resolvedSubattributeSelector !== false) {
                    resolvedAttributeSelector = properties_1.setWithinAttributeSelector(resolvedAttributeSelector, name, resolvedSubattributeSelector);
                }
            }
        }
        if (utilities_1.isComponentInstance(this) &&
            alwaysIncludePrimaryIdentifierAttributes &&
            ...
            if (isNotFilteredOut && (!setAttributesOnly || primaryIdentifierAttribute.isSet())) {
                resolvedAttributeSelector = properties_1.setWithinAttributeSelector(resolvedAttributeSelector, primaryIdentifierAttribute.getName(), true);
            }
        }
    ...
    validate(attributeSelector = true) {
        ...
        const details = failedValidators
            ...
            .join(', ');
        const error = Object.assign(new Error(`The following error(s) occurred while validating the component '${utilities_1.ensureComponentClass(this).getComponentName()}': ${details}`), { failedValidators });
        throw error;
    }
    /**
    ...
    runValidators(attributeSelector = true) {
        ...
        for (const attribute of this.getAttributes({ setAttributesOnly: true })) {
            ...
            for (const { validator, path } of attributeFailedValidators) {
                failedValidators.push({ validator, path: utilities_1.joinAttributePath([name, path]) });
            }
        }
    ...
    __getMethod(name, options) {
        ...
        if (!properties_1.isMethodInstance(property)) {
    ...
    setMethod(name, methodOptions = {}) {
        return this.setProperty(name, properties_1.Method, methodOptions);
    }
    /**
    ...
    static __getComponentOfType(type) {
        const isComponentClassType = utilities_1.assertIsComponentType(type) === 'componentClassType';
        const componentName = isComponentClassType
            ? utilities_1.getComponentNameFromComponentClassType(type)
            : utilities_1.getComponentNameFromComponentInstanceType(type);
    ...
    static getProvidedComponent(name) {
        ...
        if (!core_helpers_1.hasOwnProperty(providedComponents, name)) {
    ...
    static provideComponent(component) {
        utilities_1.assertIsComponentClass(component);
        const providedComponents = this.__getProvidedComponents();
        ...
        if (componentName in this) {
            ...
            if (!(utilities_1.isComponentClass(value) && (value === component || value.isForkOf(component)))) {
    ...
    static __getComponentProvider() {
        return core_helpers_1.hasOwnProperty(this, '__componentProvider') ? this.__componentProvider : undefined;
    }
    ...
    static __getProvidedComponents() {
        ...
        else if (!core_helpers_1.hasOwnProperty(this, '__providedComponents')) {
            Object.defineProperty(this, '__providedComponents', {
    ...
    static __getConsumedComponents(autoFork = false) {
        ...
        else if (autoFork && !core_helpers_1.hasOwnProperty(this, '__consumedComponents')) {
            Object.defineProperty(this, '__consumedComponents', {
    ...
    clone(options = {}) {
        ...
        for (const attribute of this.getAttributes({ setAttributesOnly: true })) {
            ...
            const clonedValue = cloning_1.clone(value, options);
            clonedComponent.getAttribute(name).setValue(clonedValue, { source });
        }
        return possibly_async_1.possiblyAsync(clonedComponent.initialize(), () => clonedComponent);
    }
    // === Forking ===
    /**
    ...
    fork(options = {}) {
        ...
        if (componentClass === undefined) {
            ...
            utilities_1.assertIsComponentClass(componentClass);
        }
    ...
    static isForkOf(component) {
        utilities_1.assertIsComponentClass(component);
        return core_helpers_1.isPrototypeOf(component, this);
    }
    /**
     ...
     */
    isForkOf(component) {
        utilities_1.assertIsComponentInstance(component);
        return core_helpers_1.isPrototypeOf(component, this);
    }
    static [Symbol.hasInstance](instance) {
        ...
        return instance.constructor === this || core_helpers_1.isPrototypeOf(this, instance.constructor);
    }
    /**
    ...
    static merge(forkedComponent, options = {}) {
        utilities_1.assertIsComponentClass(forkedComponent);
        if (!core_helpers_1.isPrototypeOf(this, forkedComponent)) {
    ...
    merge(forkedComponent, options = {}) {
        utilities_1.assertIsComponentInstance(forkedComponent);
        if (!core_helpers_1.isPrototypeOf(this, forkedComponent)) {
            throw new Error('Cannot merge a component that is not a fork of the target component');
        }
        this.__mergeAttributes(forkedComponent, options);
    ...
    __mergeAttributes(forkedComponent, options) {
        for (const forkedAttribute of forkedComponent.getAttributes()) {
            ...
            const mergedValue = merging_1.merge(value, forkedValue, options);
    ...
    static __serialize(options) {
        ...
        return possibly_async_1.possiblyAsync(this.__serializeAttributes(serializedComponent, options), (attributeCount) => ignoreEmptyComponents && attributeCount === 0 ? undefined : serializedComponent);
    }
    /**
    ...
    __serialize(options) {
        ...
        return possibly_async_1.possiblyAsync(this.__serializeAttributes(serializedComponent, options), (attributeCount) => ignoreEmptyComponents && attributeCount <= this.__getMinimumAttributeCount()
            ? undefined
            : serializedComponent);
    }
    ...
    __serializeAttributes(serializedComponent, options) {
        ...
        return possibly_async_1.possiblyAsync(possibly_async_1.possiblyAsync.forEach(this.getAttributes({ attributeSelector }), (attribute) => {
            const attributeName = attribute.getName();
            const subattributeSelector = properties_1.getFromAttributeSelector(attributeSelector, attributeName);
            return possibly_async_1.possiblyAsync(attributeFilter !== undefined ? attributeFilter.call(this, attribute) : true, (isNotFilteredOut) => {
                if (isNotFilteredOut) {
                    return possibly_async_1.possiblyAsync(attribute.serialize({
                        ...options,
                        attributeSelector: subattributeSelector,
    ...
    static recreate(object = {}, options = {}) {
        ...
        return possibly_async_1.possiblyAsync(component, (component) => possibly_async_1.possiblyAsync(component.__deserializeAttributes(attributes, options), () => {
            if (isNew) {
            ...
            return possibly_async_1.possiblyAsync(component.initialize(), () => component);
        }));
    }
    /**
    ...
    static deserialize(object = {}, options = {}) {
        ...
        return possibly_async_1.possiblyAsync(this.__deserializeAttributes(attributes, options), () => possibly_async_1.possiblyAsync(this.initialize(), () => this));
    }
    /**
    ...
    __deserializeAttributes(serializedAttributes, options) {
        ...
        const componentClass = utilities_1.ensureComponentClass(this);
        const componentGetter = (type) => componentClass.getComponentOfType(type);
        return possibly_async_1.possiblyAsync.forEach(Object.entries(serializedAttributes), ([attributeName, serializedAttributeValue]) => {
    ...
    __deserializeAttribute(attribute, serializedAttributeValue, componentGetter, options) {
        ...
        return possibly_async_1.possiblyAsync(deserialization_1.deserialize(serializedAttributeValue, { ...options, componentGetter }), (newAttributeValue) => {
            if (attribute.isSet()) {
                ...
                if (utilities_1.isComponentClassOrInstance(newAttributeValue) &&
                    utilities_1.isComponentClassOrInstance(previousAttributeValue) &&
    ...
    static __introspectMixins() {
        ...
        while (utilities_1.isComponentClass(currentClass)) {
            if (core_helpers_1.hasOwnProperty(currentClass, '__mixin')) {
    ...
    static __unintrospectMixins(introspectedMixins, { mixins }) {
        ...
        for (const mixinName of introspectedMixins) {
            const Mixin = mixins.find((Mixin) => core_helpers_1.getFunctionName(Mixin) === mixinName);
            if (Mixin === undefined) {
    ...
    __unintrospectProperties(introspectedProperties, propertyClassGetter, { methodBuilder }) {
        ...
        for (const introspectedProperty of introspectedProperties) {
            ...
            const PropertyClass = propertyClassGetter.call(utilities_1.ensureComponentClass(this), type);
            ...
            else if (properties_1.isMethodInstance(property)) {
    ...
    callRemoteMethod(name, ...args) {
        const remoteMethodBuilder = utilities_1.ensureComponentClass(this).__remoteMethodBuilder;
    ...
    static isComponent(value) {
        return utilities_1.isComponentInstance(value);
    }
    static get toObject() {
    ...
    toObject(options = {}) {
        ...
        if (minimize) {
            if (utilities_1.isComponentClass(this)) {
                return {};
            }
        ...
        const handleValue = (value) => {
            if (utilities_1.isComponentClassOrInstance(value)) {
                ...
                if (!utilities_1.ensureComponentClass(component).isEmbedded()) {
    ...
    describeComponent(options = {}) {
        ...
        return `${componentPrefix}component: '${utilities_1.ensureComponentClass(this).getComponentPath()}'`;
    }
...
function createPropertyFilter(originalFilter, options = {}) {
    ...
    const normalizedAttributeSelector = properties_1.normalizeAttributeSelector(attributeSelector);
    const filter = function (property) {
        if (properties_1.isAttributeInstance(property)) {
            ...
            const name = attribute.getName();
            if (properties_1.getFromAttributeSelector(normalizedAttributeSelector, name) === false) {
                return false;
            }
        }
