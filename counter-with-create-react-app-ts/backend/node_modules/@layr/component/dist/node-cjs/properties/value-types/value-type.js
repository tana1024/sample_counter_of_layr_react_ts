...
class ValueType {
    constructor(attribute, options = {}) {
        ...
        const normalizedValidators = validators.map((validator) => validation_1.normalizeValidator(validator, attribute));
    ...
    checkValue(value, attribute) {
        if (!this._checkValue(value, attribute)) {
            throw new Error(`Cannot assign a value of an unexpected type (${attribute.describe()}, expected type: '${this.toString()}', received type: '${core_helpers_1.getTypeOf(value)}')`);
        }
    }
    _checkValue(value, _attribute) {
        return value === undefined ? this.isOptional() : undefined;
    }
    _traverseAttributes(_iteratee, _attribute, _value, _options) {
        // NOOP
    }
    _resolveAttributeSelector(normalizedAttributeSelector, _attribute, _value, _options) {
    ...
    runValidators(value, _attributeSelector) {
        const failedValidators = validation_1.runValidators(this.getValidators(), value);
    ...
    serializeValue(value, _attribute, options = {}) {
        return serialization_1.serialize(value, options);
    }
    introspect() {
        const introspectedValueType = { valueType: this.toString() };
