...
exports.isAttributeInstance = exports.isAttributeClass = exports.Attribute = void 0;
...
const forking_1 = require("../forking");
 ...
 * ```
 */
class Attribute extends observable_1.Observable(property_1.Property) {
    /**
    ...
    setOptions(options = {}) {
        ...
        this._valueType = value_types_1.createValueType(valueType, this, { validators, items });
    ...
    getValue(options = {}) {
        ...
        if (autoFork && !core_helpers_1.hasOwnProperty(this, '_value')) {
            const parent = this.getParent();
            const value = this._value;
            const componentClass = utilities_1.isComponentInstance(value)
                ? utilities_1.ensureComponentClass(parent).getComponent(value.constructor.getComponentName())
                : undefined;
            let forkedValue = forking_1.fork(value, { componentClass });
            if (observable_1.canBeObserved(forkedValue)) {
                if (!observable_1.isObservable(forkedValue)) {
                    forkedValue = observable_1.createObservable(forkedValue);
                }
                if (observable_1.isEmbeddable(forkedValue)) {
    ...
    setValue(value, { source = 0 } = {}) {
        ...
        if (observable_1.canBeObserved(value) && !observable_1.isObservable(value)) {
            value = observable_1.createObservable(value);
        }
        ...
        if (valueHasChanged) {
            if (observable_1.isObservable(previousValue) && observable_1.isEmbeddable(previousValue)) {
                previousValue.removeObserver(this);
            }
            if (observable_1.isObservable(value) && observable_1.isEmbeddable(value)) {
    ...
    evaluateDefault() {
        ...
        if (typeof value === 'function' && !utilities_1.isComponentClass(value)) {
    ...
    introspect() {
        ...
        const getIsExposed = exposure !== undefined ? core_helpers_1.hasOwnProperty(exposure, 'get') : false;
        const setIsExposed = exposure !== undefined ? core_helpers_1.hasOwnProperty(exposure, 'set') : false;
    ...
    static unintrospect(introspectedAttribute) {
        ...
        Object.assign(options, value_types_1.unintrospectValueType({ valueType, validators, items }));
        return { name, options };
    }
    // === Utilities ===
 ...
 * @category Utilities
 */
function isAttributeClass(value) {
    return typeof (value === null || value === void 0 ? void 0 : value.isAttribute) === 'function';
}
exports.isAttributeClass = isAttributeClass;
/**
 ...
 * @category Utilities
 */
function isAttributeInstance(value) {
    return isAttributeClass(value === null || value === void 0 ? void 0 : value.constructor) === true;
}
exports.isAttributeInstance = isAttributeInstance;
